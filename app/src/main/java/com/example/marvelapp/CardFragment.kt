...
class CardFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        binding.contcard.setOnClickListener{
            if (binding.firstname.text!!.isEmpty()){
                Toast.makeText(requireContext(), "Enter first name", Toast.LENGTH_LONG).show()
            }else{
                if (binding.cardnumber.text!!.isEmpty()){
                    Toast.makeText(requireContext(), "Enter card number", Toast.LENGTH_LONG).show()
                }else{
                        findNavController().navigate(R.id.action_cardFragment_to_menuHomeFragment)
                    }
                }
            }
        return binding.root
